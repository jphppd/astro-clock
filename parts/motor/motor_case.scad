...
module main_case(wall_width) translate([0, shaft_position, 0])
    rotate(90)
      union() {
        translate([0, 0, -motor_height])
          difference() {
            ...
            translate([0, 0, motor_height - screw_plate_height])
              linear_extrude(screw_plate_height)
                screw_plate();
        ...
        // Pins
        place_pins()
          translate([0, 0, -screw_plate_height - eps])
            cylinder(h=2 * screw_plate_height + eps, r1=screwhole_radius - eps, r2=0.92 * screwhole_radius);
      }
...
module main_cap() translate([0, shaft_position, 0])
    rotate(90) {
      difference() {
        union() {
          linear_extrude(2 * screw_plate_height)
            screw_plate_case();
          cylinder(r=motor_outer_diameter_top / 2 + wall_width, h=2 * screw_plate_height);
        }
        place_pins()
