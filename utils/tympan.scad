...
function fn_pop_last(list) =
  len(list) > 1 ? [
      for (idx = [0:max(0, len(list) - 2)]) list[idx],
    ]
  : [];
function fn_pop_first(list) =
  len(list) > 1 ? [
      for (idx = [1:len(list) - 1]) list[idx],
    ]
  : [];
function fn_pop_first_last(list) =
  len(list) > 2 ? [
      for (idx = [1:max(0, len(list) - 2)]) list[idx],
    ]
  : [];
function fn_reverse(list) =
  let (length = len(list)) [
      for (idx = [1:length]) list[length - idx],
  ];
...
function fn_circle_intersection_angles(circle_main, intersct) =
  ...
  : let (
    ...
    r2 = sqrt(pow(rcos2, 2) + pow(rsin2, 2)),
    cos1 = rcos1 / r1, //
    cos2 = rcos2 / r2, //
    sin1 = rsin1 / r1, //
    sin2 = rsin2 / r2
...
function fn_interpolate_angles(list, reverse = false) =
  concat(
    //
    [
      for (angles = fn_pairwise(list)) each [
        //
        angles[0], //
...
function arc(center_and_r, thetas, reverse = undef) =
  ...
  [
      for (theta = interpolate_angles(thetas, reverse=do_reverse)) point_on_arc(center_and_r, theta),
  ];

function arc_east_to_west(center_and_r, thetas, reverse = undef) =
  //
  let (
    points = arc(center_and_r, thetas, reverse), //
...
function almucantar_flat_sector(obs_latitude, alt1, alt2, scale_factor = 1) =
  let (
    ...
    almucantar_zenith = fn_almucantar(obs_latitude, max(alt1, alt2), scale_factor),
    almucantar_nadir = fn_almucantar(obs_latitude, min(alt1, alt2), scale_factor),
    tropic_of_capricorn = fn_proj_circle([0, 0, 1], sin(-ecliptic_inclination), scale_factor),
    tropic_of_cancer = fn_proj_circle([0, 0, 1], sin(+ecliptic_inclination), scale_factor),
    zenith_cancer = fn_circle_intersection(almucantar_zenith, tropic_of_cancer),
    nadir_cancer = fn_circle_intersection(almucantar_nadir, tropic_of_cancer),
    thetas_zenith = fn_circle_intersection_angles(almucantar_zenith, zenith_cancer),
    thetas_nadir = fn_circle_intersection_angles(almucantar_nadir, nadir_cancer),
    thetas_cancer_zenith = fn_circle_intersection_angles(tropic_of_cancer, zenith_cancer),
    thetas_cancer_nadir = fn_circle_intersection_angles(tropic_of_cancer, nadir_cancer),
    thetas_cancer_west = [is_undef(thetas_zenith) ? [1, 0] : thetas_cancer_zenith[0], is_undef(thetas_nadir) ? [-1, 0] : thetas_cancer_nadir[0]],
    thetas_cancer_east = [is_undef(thetas_zenith) ? [1, 0] : thetas_cancer_zenith[1], is_undef(thetas_nadir) ? [-1, 0] : thetas_cancer_nadir[1]],
    almucantar_zenith_half_arc = is_undef(thetas_zenith) ? [] : arc_east_to_west(almucantar_zenith, thetas_zenith),
    almucantar_nadir_half_arc = is_undef(thetas_nadir) ? [] : arc_west_to_east(almucantar_nadir, thetas_nadir),
    cancer_west = arc_north_to_south(tropic_of_cancer, thetas_cancer_west, reverse=false),
    cancer_east = arc_south_to_north(tropic_of_cancer, thetas_cancer_east, reverse=false)
