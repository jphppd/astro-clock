...
function fn_rotate(points, angle) =
  ...
  [
      for (point = points) is_undef(point.z) ? //
...
function root_arc_when_no_undercut(r_dedendum, delta_phi, phib) =
  //
  [
    for (angle = [delta_phi, phib]) [r_dedendum * cos(angle), r_dedendum * sin(angle)],
  ];
...
function fn_gear_tooth(
  ...
  let (
    ...
    tooth_and_roots = concat(
      ...
      [
        for (pt = tooth_and_roots) [pt.x, -pt.y],
      ],
...
function fn_gear_section(n) =
  ...
  [
      [
        for (i = [0:n - 1]) each fn_pop_last(fn_rotate(tooth, i * delta_theta)),
      ],
      gt[1],
      gt[2],
  ];
...
function fn_lid_teeth_faces(layers_nb, layer_stride, teeth_nb, tooth_stride, root_pts_nb, flank_pts_nb) =
  let (
    ...
    bottom_lid_teeth = [
      for (tooth_idx = [0:teeth_nb - 1]) each //
      let (
        //
      ...
      concat(
        //
        [
          for (idx = flanks) [offset + pitch, offset + idx[0], offset + idx[1]],
        ], //
        [
          for (idx = right_root_a) [offset + dedendum, offset + idx[0], offset + idx[1]],
        ], //
        [
          for (idx = right_root_b) [offset + base, offset + idx[0], offset + idx[1]],
        ], //
        [
          for (idx = left_root_b) [offset + base, offset + idx[0], offset + idx[1]],
        ], //
        [
          for (idx = left_root_a) [offset + dedendum, offset + idx[0], offset + idx[1]],
        ], //
        [
    ...
    top_lid_teeth = [
      for (triangle = bottom_lid_teeth) [
        for (pt = fn_reverse(triangle)) pt + (layers_nb - 1) * layer_stride,
      ],
    ] //
...
function fn_lid_inner_faces(layers_nb, layer_stride, teeth_nb, tooth_stride, root_pts_nb, flank_pts_nb) =
  let (
    ...
    left_root_end = fn_left_root_end(root_pts_nb, flank_pts_nb), //
    dedendum = fn_tooth_dedendum(root_pts_nb, flank_pts_nb), //
    bottom_lid_inner_tooth = [right_root_begin, dedendum, tooth_stride],
    bottom_lid_inner = [
      for (tooth_idx = [0:teeth_nb - 1], pt = bottom_lid_inner_tooth) //
      (pt + tooth_idx * tooth_stride) % layer_stride,
    ],
    top_lid_inner = [
      for (pt = fn_reverse(bottom_lid_inner)) pt + (layers_nb - 1) * layer_stride,
    ] //
...
function fn_faces(layers_nb, layer_stride, teeth_nb, tooth_stride, root_pts_nb, flank_pts_nb) =
  let (
    //
    flank_faces = fn_flank_faces(layers_nb, layer_stride, teeth_nb, tooth_stride, 2 * (root_pts_nb + flank_pts_nb + 1)),
    lid_teeth_faces = fn_lid_teeth_faces(layers_nb, layer_stride, teeth_nb, tooth_stride, root_pts_nb, flank_pts_nb),
...
function fn_gear(n, m, layers, bevel_cone_angle = 0) =
  let (
    ...
    extruded_profile = [
      for (layer_idx = [0:len(layers) - 1], i = [0:n - 1]) each fn_rotate(tooth_per_layer[layer_idx], i * delta_theta),
    ],
...
function fn_gear_profile_partial(n, n_partial, $fn = 32) =
  let (
    ...
    teeth = [
      for (i = [-(n_partial - 1) / 2:n_partial / 2]) each fn_pop_last(fn_rotate(tooth, i * 360 / n)),
    ], //
...
function fn_anti_gear(n, m, $fn = 32) =
  let (
    ...
    flank_left = [
      for (pt = flank) [c * pt.x + s * pt.y, -s * pt.x + c * pt.y],
    ], //
...
module anti_gear(n, m, thickness, belt_thickness = 0) linear_extrude(thickness)for (theta = [0:360 / n:360])
    rotate(theta)
      difference() {
