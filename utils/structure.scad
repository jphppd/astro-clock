...
function annulus_middle(n) =
  concat(
    //
    [
      for (i = [0:n]) hub_radius + annulus_sagitta / 2 + //
...
module hub_outer() polygon(
    concat(
      //
      [
        for (theta = [-36:2:36]) [hub_radius * cos(theta), hub_radius * sin(theta)],
      ], [
        for (theta = [-36:2:36]) [hub_split * hub_radius * cos(-theta), hub_split * hub_radius * sin(-theta)],
      ]
    )
  );
...
module dovetails() rotate(-36)for (dx = annulus_middle(nb_of_annuli))
    translate([dx, 0, 0])
